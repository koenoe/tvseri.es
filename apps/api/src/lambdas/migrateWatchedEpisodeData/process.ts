...
const seasonCache = new Map<string, Map<number, { airDate: string }>>();

const getSeasonData = async (
  seriesId: number,
  seasonNumber: number,
): Promise<Map<number, { airDate: string }> | null> => {
  ...
  try {
    ...
    const episodeMap = new Map<number, { airDate: string }>();
...
const updateWatchedItem = async (
  ...
  const command = new UpdateItemCommand({
    ...
    UpdateExpression: 'SET #episodeAirDate = :episodeAirDate',
  });
...
export const handler: SQSHandler = async (event): Promise<SQSBatchResponse> => {
  ...
  for (const record of event.Records) {
    try {
      ...
      console.log(
        `Updated: series ${message.seriesId} S${message.seasonNumber}E${message.episodeNumber} -> airDate: "${episodeData.airDate}"`,
      );
    } catch (error) {
